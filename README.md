...
Like all Minipass streams, Glob objects start out in a "paused" state, and
...
* `noglobstar` Do not match `**` against multiple filenames.  (Ie, treat it
  as a normal `*` instead.)
...
Glob's logo was created by [Tanya Brassie](http://tanyabrassie.com/). Logo
files can be found
